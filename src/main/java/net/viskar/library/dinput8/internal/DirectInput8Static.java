...
final class DirectInput8Static {
    ...
    public static int EnumDevices(int dwDevType, Addressable pvRef, int dwFlags, EnumDevicesCallback deviceCallback) {

        Callback callback = prevEnumDevices;
        if (callback == null) {
            prevEnumDevices = callback = new Callback(deviceCallback, deviceCallback.allocate().share());
        } else if (callback.impl != deviceCallback) {
            callback.segment.close();
            prevEnumDevices = callback = new Callback(deviceCallback, deviceCallback.allocate().share());
        }

        return EnumDevices(dwDevType, callback.segment.address(), pvRef.address(), dwFlags);
    }
